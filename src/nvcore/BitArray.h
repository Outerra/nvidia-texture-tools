...
public:
    ...
    void clearAll()
    {
        memset(&m_bitArray[0], 0, m_bitArray.size());
    }

    /// Set all the bits.
    void setAll()
    {
        memset(&m_bitArray[0], 0xFF, m_bitArray.size());
    }

    /// Toggle all the bits.
    void toggleAll()
    {
        const size_t byte_num = m_bitArray.size();
        for (size_t b = 0; b < byte_num; b++) {
            m_bitArray[b] ^= 0xFF;
        }
    }
    ...
    {
        const size_t num = m_bitArray.size();
        if (num == 0) {
            return 0;
        }
