...
void nv::fillVoronoi(FloatImage * img, const BitMap * bmap)
{
    ...
    const int w = img->width();
    const int h = img->height();
    const int count = img->componentCount()==4 ? 3 : img->componentCount();

    nvCheck(bmap->width() == uint(w));
    nvCheck(bmap->height() == uint(h));

    std::vector<Neighbor> edm;
    edm.resize(w * h);
    ...
    for(int y=0; y<h; ++y)
        for(int x=0; x<w; ++x)
            if(!bmap->bitAt(x,y)) {
                ...
                color[0]+=img->pixel(0, img->indexClamp(x-1,y-1,0))*weights[0];
                color[1]+=img->pixel(1, img->indexClamp(x-1,y-1,0))*weights[0];
                color[2]+=img->pixel(2, img->indexClamp(x-1,y-1,0))*weights[0];

                color[0]+=img->pixel(0, img->indexClamp(x,y-1,0))*weights[1];
                color[1]+=img->pixel(1, img->indexClamp(x,y-1,0))*weights[1];
                color[2]+=img->pixel(2, img->indexClamp(x,y-1,0))*weights[1];

                color[0]+=img->pixel(0, img->indexClamp(x+1,y-1,0))*weights[2];
                color[1]+=img->pixel(1, img->indexClamp(x+1,y-1,0))*weights[2];
                color[2]+=img->pixel(2, img->indexClamp(x+1,y-1,0))*weights[2];

                color[0]+=img->pixel(0, img->indexClamp(x-1,y,0))*weights[3];
                color[1]+=img->pixel(1, img->indexClamp(x-1,y,0))*weights[3];
                color[2]+=img->pixel(2, img->indexClamp(x-1,y,0))*weights[3];

                color[0]+=img->pixel(0, img->indexClamp(x,y,0))*weights[4];
                color[1]+=img->pixel(1, img->indexClamp(x,y,0))*weights[4];
                color[2]+=img->pixel(2, img->indexClamp(x,y,0))*weights[4];

                color[0]+=img->pixel(0, img->indexClamp(x+1,y,0))*weights[5];
                color[1]+=img->pixel(1, img->indexClamp(x+1,y,0))*weights[5];
                color[2]+=img->pixel(2, img->indexClamp(x+1,y,0))*weights[5];

                color[0]+=img->pixel(0, img->indexClamp(x-1,y+1,0))*weights[6];
                color[1]+=img->pixel(1, img->indexClamp(x-1,y+1,0))*weights[6];
                color[2]+=img->pixel(2, img->indexClamp(x-1,y+1,0))*weights[6];

                color[0]+=img->pixel(0, img->indexClamp(x,y+1,0))*weights[7];
                color[1]+=img->pixel(1, img->indexClamp(x,y+1,0))*weights[7];
                color[2]+=img->pixel(2, img->indexClamp(x,y+1,0))*weights[7];

                color[0]+=img->pixel(0, img->indexClamp(x+1,y+1,0))*weights[8];
                color[1]+=img->pixel(1, img->indexClamp(x+1,y+1,0))*weights[8];
                color[2]+=img->pixel(2, img->indexClamp(x+1,y+1,0))*weights[8];

                img->pixel(0, x, y, 0) = color[0];
                img->pixel(1, x, y, 0) = color[1];
                img->pixel(2, x, y, 0) = color[2];
                img->pixel(2, x, y, 0) = 0;
            }
}
...
static bool downsample(const FloatImage * src, const BitMap * srcMask, const FloatImage ** _dst, const BitMap ** _dstMask)
{
    ...
    const uint count = src->componentCount();
    ...
    for(uint c = 0; c < count; c++) {
        for(uint y = 0; y < nh; y++) {
            for(uint x = 0; x < nw; x++) {
                ...
                const float f0 = src->pixel(c, x0, y0, 0);
                const float f1 = src->pixel(c, x1, y0, 0);
                const float f2 = src->pixel(c, x0, y1, 0);
                const float f3 = src->pixel(c, x1, y1, 0);
...
void nv::fillPullPush(FloatImage * img, const BitMap * bmap)
{
    ...
    const uint count = img->componentCount();
    const uint w = img->width();
    const uint h = img->height();
    const uint num = log2(max(w,h));

    // Build mipmap chain.
    std::vector<const FloatImage *> mipmaps(num);
    std::vector<const BitMap *> mipmapMasks(num);

    mipmaps.push_back(img);
    mipmapMasks.push_back(bmap);
    ...
    while(downsample(mipmaps.back(), mipmapMasks.back(), &current, &currentMask))
    {
        ...
        mipmapMasks.push_back(currentMask);
    }
    ...
    for(uint y = 0; y < h; y++) {
        for(uint x = 0; x < w; x++) {
            ...
            const size_t levelCount = mipmaps.size();
            for (size_t l = 0; l < levelCount; l++)
            {
                ...
                if (mipmapMasks[l]->bitAt(sx, sy))
                {
                    ...
                    for(uint c = 0; c < count; c++) {
                        ...
                        img->pixel(c, x, y, 0) = mipmaps[l]->pixel(c, sx, sy, 0);
                    }
                    break;
                }
